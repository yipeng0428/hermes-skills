...
## ⚠️ 关键教训：递归自愈 bug（2026-08-01 真实事故）

**事故**: v2.0 把 `scan_errors_log()` 扫到的**活跃会话**也当成"中断会话"恢复，
把「【自动恢复】」指令注入**正在运行的当前会话** → 新 API 调用 → 又失败 → 又注入
→ 无限递归！2 分钟 cron + 15 分钟 cron 并发执行，多个 `hermes chat --continue`
进程同时往一个会话注入消息，会话被污染、系统卡死。

**修复 (v2.1)**:
1. **活跃会话保护** `is_active_session()`: `end_reason IS NULL` 且最近 120 秒
   有消息 = 正在交互 → `resume_session()` 直接返回 "会话活跃中, 跳过自动恢复"
2. **单实例锁** `acquire_lock()`: 锁文件 `tmp/auto_heal.lock`，30 分钟过期兜底。
   多个 cron 并发时只有第一个执行，其余静默退出
3. **fast 模式不恢复会话**: `--fast` 只扫日志摘死模型 + 切主模型（秒级），
   恢复动作只留给 `--heal` 深度模式
4. 错误会话判定必须排除活跃会话：日志中出现的 session 如果 end_reason=NULL
   且最近有消息，不能恢复
